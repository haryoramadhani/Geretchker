...
$testdrive = query3("SELECT * FROM test_drive");
 
 if(isset($_POST["cari3"]))
 {
 	$testdrive = cari3($_POST["keyword3"]);
 }
 ?>
...
<body style="background-color: rgb(237, 236, 251);">
  ...
  <nav class="navbar navbar-dark bg-secondary">
      ...
      <div style="margin-top: -40px; margin-left: 440px;"><h1><img style="width: 45px; margin-top: -10px;" src="svg/baseline-person_outline-24px.svg"> Menu Member</h1></div>
      <div><form class="form-inline" style=" margin-top: -50px; margin-left: 910px;" action="" method="post">
		<input class="form-control mr-sm-2" type="text" name="keyword3" size="40" autofocus placeholder="masukkan keyword pencarian.." autocomplete="off" id="keyword3">
		<button id="tombol-cari" class="btn btn-outline-light my-2 my-sm-0" type="submit" name="cari3"><img src="svg/baseline-search-24px.svg"/></button>
	</form></div>
      <div style="margin-left: -20px;" class="dropdown-menu bg-secondary text">
        ...
        <p style="margin-left: 30px;"><img src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"></p>
          <a style="margin-top: -20px;" class="dropdown-item text-light" href="tambahtestdrive.php"><img style="" src="svg/outline-add_box-24px.svg"/>Input TestDrive</a>
        <p style="margin-left: 30px;"><img src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"><img style="margin-left: -6px;" src="svg/baseline-more_horiz-24px.svg"></p>
	...
	<?php $i = 1; ?>
	<?php foreach ($testdrive as $row) : ?>
	<tr>
		
		<td><a class="btn btn-primary" href="ubahtestdrive.php?no_test=<?= $row["no_test"]; ?>"><img src="svg/baseline-update-24px.svg"/>ubah</a> <a class="btn btn-danger" href="hapustestdrive.php?no_test=<?= $row["no_test"]; ?>" onclick="return confirm('apakah anda yakin akan menghapus data tersebut?')"><img src="svg/baseline-delete_outline-24px.svg"/>hapus</a></td>
	
		
		<td><?= $row["waktu_test"]; ?></td>
		<td><?= $row["no_test"]; ?> </td>
		<td><?= $row["tgl_test"]; ?></td>
